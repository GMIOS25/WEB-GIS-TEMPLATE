...
@Testcontainers
class AgricultureControllerIntegrationTest {
    ...
    void setUp() throws Exception {
        agricultureUnitRepository.deleteAll();
    ...
    @Test
    void testAgricultureCrudLifecycle_withDatabase() throws Exception {
        ...
        AgricultureUnitCreateRequest createRequest = AgricultureUnitCreateRequest.builder()
                .name("Trang trại Trồng trọt")
                .unitType("Trang trại")
                .description("Mô hình nông nghiệp công nghệ cao")
        ...
        MvcResult createResult = mockMvc.perform(post("/api/agriculture")
                        .cookie(adminCookie)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(createRequest)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.name").value("Trang trại Trồng trọt"))
        ...
        mockMvc.perform(get("/api/agriculture").cookie(viewerCookie))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray())
                .andExpect(jsonPath("$.totalElements").value(1));

        mockMvc.perform(get("/api/agriculture/" + createdId).cookie(viewerCookie))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(createdId))
                .andExpect(jsonPath("$.name").value("Trang trại Trồng trọt"));

        // 4. Admin can delete (200)
        mockMvc.perform(delete("/api/agriculture/" + createdId).cookie(adminCookie))
                .andExpect(status().isOk());
