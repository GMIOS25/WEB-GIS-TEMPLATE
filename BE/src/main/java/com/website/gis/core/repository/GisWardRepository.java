...
public interface GisWardRepository extends JpaRepository<GisWard, Integer> {
    ...
    @Query(value = "SELECT geojson FROM v_wards_geojson", nativeQuery = true)
    Optional<String> findWardsFeatureCollection();
    ...
    @Query(value = "SELECT geojson FROM v_province_geojson", nativeQuery = true)
